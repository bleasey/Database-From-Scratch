...
void* Pager::get_page(uint32_t page_num) {
  uint32_t num_pages = this->file_length / DB::PAGE_SIZE;
  uint32_t page_offset = this->file_length % DB::PAGE_SIZE;

  // Adding a page when previous is full
  if (page_num == num_pages && page_offset == 0) {
    this->pages[page_num] = malloc(DB::PAGE_SIZE);
  ...
  if (this->pages[page_num] == NULL) {
    void* page = malloc(DB::PAGE_SIZE);
    lseek(this->file_descriptor, page_num * DB::PAGE_SIZE, SEEK_SET);
    ssize_t bytes_read = read(this->file_descriptor, page, DB::PAGE_SIZE);
...
void Pager::flush_to_disk(uint32_t page_num, uint32_t size) {
  ...
  off_t offset = lseek(this->file_descriptor, page_num * DB::PAGE_SIZE, SEEK_SET);
