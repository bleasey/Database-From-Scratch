...
Table::Table() {  
  ...
  this->num_rows = pager->file_length / Table::ROW_SIZE;
  this->row_to_insert = new Row();
}

Table::~Table() {
  Pager* pager = this->pager;
  uint32_t num_full_pages = this->num_rows / Table::ROWS_PER_PAGE;
  ...
  uint32_t num_additional_rows = this->num_rows % Table::ROWS_PER_PAGE;
...
void* Table::get_row(uint32_t row_num) {
